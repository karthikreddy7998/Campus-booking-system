...
function AdminDashboard() {
  ...
  const fetchBookings = async () => {
    try {
      const res = await fetch("https://campus-booking-system-1-nqej.onrender.com/api/bookings", { cache: "no-store" });
  ...
  const handleStatus = async (id, action) => {
    try {
      const res = await fetch(`https://campus-booking-system-1-nqej.onrender.com/api/bookings/${action}/${id}`, { method: "PUT" });
      if (res.ok) {
  ...
  const handleExport = (type) => {
    window.open(`https://campus-booking-system-1-nqej.onrender.com/api/export/${type}`, '_blank');
  };
